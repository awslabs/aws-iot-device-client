    ...
    namespace Iot
    {
        namespace DeviceClient
        {
            namespace DeviceDefender
            {
                ...
                string DeviceDefenderFeature::getName() { return "Device Defender"; }
                ...
                void DeviceDefenderFeature::startDeviceDefender()
                {
                    ...
                    task = unique_ptr<Iotdevicedefenderv1::ReportTask>(
                        new Iotdevicedefenderv1::ReportTask(taskBuilder.Build()));
                    LOGM_DEBUG(TAG, "%s task build finished", getName().c_str());
                    task->StartTask();
                    LOGM_DEBUG(TAG, "%s StartTask() async called", getName().c_str());

                    auto onRecvData =
                        [&](MqttConnection &connection, const String &topic, const ByteBuf &payload) -> void {
                        LOGM_DEBUG(TAG, "Recv: Topic:(%s), Payload:%s", topic.c_str(), (char *)payload.buffer);
                    };
                    auto onSubAck = [&](MqttConnection &connection,
                                        uint16_t packetId,
                                        const String &topic,
                                        QOS qos,
                                        int errorCode) -> void {
