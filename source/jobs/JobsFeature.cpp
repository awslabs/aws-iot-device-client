...
void JobsFeature::publishStartNextPendingJobExecutionRequest() {
    ...
    jobsClient->PublishStartNextPendingJobExecution(startNextRequest, AWS_MQTT_QOS_AT_LEAST_ONCE,
...
void JobsFeature::subscribeToStartNextPendingJobExecution() {
    ...
    jobsClient->SubscribeToStartNextPendingJobExecutionAccepted(startNextSub, AWS_MQTT_QOS_AT_LEAST_ONCE,
...
std::bind(&JobsFeature::ackSubscribeToStartNextJobAccepted, this, std::placeholders::_1));
    jobsClient->SubscribeToStartNextPendingJobExecutionRejected(startNextSub, AWS_MQTT_QOS_AT_LEAST_ONCE,
...
void JobsFeature::subscribeToNextJobChangedEvents() {
    ...
    jobsClient->SubscribeToNextJobExecutionChangedEvents(nextJobSub, AWS_MQTT_QOS_AT_LEAST_ONCE,
...
void JobsFeature::subscribeToUpdateJobExecutionStatusAccepted(string jobId) {
    ...
    jobsClient->SubscribeToUpdateJobExecutionAccepted(request, AWS_MQTT_QOS_AT_LEAST_ONCE,
std::bind(&JobsFeature::updateJobExecutionStatusAcceptedHandler, this, std::placeholders::_1, std::placeholders::_2),
...
void JobsFeature::subscribeToUpdateJobExecutionStatusRejected(string jobId) {
    ...
    request.ThingName = thingName.c_str();
    request.JobId = jobId.c_str();
    jobsClient->SubscribeToUpdateJobExecutionRejected(request, AWS_MQTT_QOS_AT_LEAST_ONCE,
      std::bind(&JobsFeature::updateJobExecutionStatusRejectedHandler, this, std::placeholders::_1, std::placeholders::_2),
...
void JobsFeature::startNextPendingJobReceivedHandler(StartNextJobExecutionResponse* response, int ioError) {
    ...
    if(response->Execution.has_value()) {
        ...
        if(needStop) {
            ...
            jobMessage << "Incoming " << Iotjobs::JobStatusMarshaller::ToString(response->Execution->Status.value()) << " job: " << response->Execution->JobId->c_str();
...
void JobsFeature::nextJobChangedHandler(NextJobExecutionChangedEvent *event, int ioError) {
    ...
    if(event->Execution.has_value()) {
        ...
        if(needStop) {
            ...
            LOG_WARN(TAG, "Received new job but JobsFeature is stopped");
            ostringstream jobMessage;
            jobMessage << "Incoming " << Iotjobs::JobStatusMarshaller::ToString(event->Execution->Status.value()) << " job: " << event->Execution->JobId->c_str();
            baseNotifier->onError(this, ClientBaseErrorNotification::MESSAGE_RECEIVED_AFTER_SHUTDOWN, jobMessage.str());
            return;
        } else {
            handlingJob = true;
...
void JobsFeature::publishUpdateJobExecutionStatus(JobExecutionData data, JobExecutionStatusInfo statusInfo) {
    ...
    if(!statusInfo.stdoutput.empty() || !statusInfo.stderror.empty())  {
        if(includeStdoutInUpdates && !statusInfo.stdoutput.empty()) {
            ...
            int startPos = statusInfo.stdoutput.size() > MAX_STATUS_DETAIL_LENGTH ? statusInfo.stdoutput.size() - MAX_STATUS_DETAIL_LENGTH : 0;
            // TODO We need to add filtering of invalid characters for the status details that may come from weird process output.
            // The valid values for a statusDetail value are '[^\p{C}]+ which translates into "everything other than invisible control characters
            // and unused code points" (See http://www.unicode.org/reports/tr18/#General_Category_Property)
        ...
        if(!statusInfo.stderror.empty()) {
            int startPos = statusInfo.stderror.size() > MAX_STATUS_DETAIL_LENGTH ? statusInfo.stderror.size() - MAX_STATUS_DETAIL_LENGTH : 0;
     ...
     */
    RetryConfig retryConfig = {10 * 1000, 640 * 1000, -1, canStopLock, needStop};
    auto publishLambda = [this, data, statusInfo, statusDetails]() -> bool {
        ...
        for(auto keyPromise = updateJobExecutionPromises.cbegin(); keyPromise != updateJobExecutionPromises.cend(); /** no increment here **/) {
        ...
        this->updateJobExecutionPromises.insert(std::pair<Aws::Crt::String, EphemeralPromise<int>>(clientToken.c_str(), EphemeralPromise<int>(std::chrono::milliseconds(15 * 1000))));
        writeLock.unlock();

        this->jobsClient->PublishUpdateJobExecution(request, AWS_MQTT_QOS_AT_LEAST_ONCE, std::bind(&JobsFeature::ackUpdateJobExecutionStatus, this, std::placeholders::_1));
        ...
        if(std::future_status::timeout == updateFuture.wait_for(std::chrono::seconds(10))) {
            ...
            if(responseCode != 0) {
                LOGM_WARN(TAG, "Received a non-zero response after publishing an UpdateJobExecution request: %d", responseCode);
            } else {
...
void JobsFeature::executeJob(JobExecutionData job) {
    ...
    if(jobDoc.KeyExists(JOB_ATTR_ARGS) && jobDoc.GetJsonObject(JOB_ATTR_ARGS).IsListType()) {
        ...
        LOG_INFO(TAG, "Did not find any arguments in the incoming job document. Value should be a JSON array of arguments");
    }
    ...
    if(!executionStatus && engine->hasErrors() <= allowStdErr) {
        ...
        publishUpdateJobExecutionStatus(job, {JobStatus::FAILED, reason.str(), engine->getStdOut(), engine->getStdErr()});
    }
    ...
    if(needStop) {
        lock.unlock();
        LOGM_INFO(TAG, "Shutting down %s now that job execution is complete", get_name().c_str());
...
void JobsFeature::runJobs() {
    ...
    jobsClient = unique_ptr<IotJobsClient>(new IotJobsClient(mqttManager.get()->getConnection()));
