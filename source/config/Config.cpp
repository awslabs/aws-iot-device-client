...
int PlainConfig::LogConfig::ParseDeviceClientLogLevel(string level) {
    ...
    if ("DEBUG" == temp) {
        ...
        throw std::invalid_argument(FormatMessage(
                "Provided log level %s is not a known log level for the AWS IoT Device Client",
                Sanitize(level).c_str()));
    }
}
...
bool PlainConfig::SampleShadow::LoadFromJson(const Crt::JsonView &json) {
    ...
    if (!json.GetString(jsonKey).empty()) {
        ...
        LOGM_WARN(
                Config::TAG, "Shadow Name {%s} was provided in the JSON configuration file with an empty value",
                jsonKey);
    }
...
bool PlainConfig::SampleShadow::LoadFromCliArgs(const CliArgs &cliArgs) {
    ...
    if (cliArgs.count(PlainConfig::SampleShadow::CLI_SAMPLE_SHADOW_INPUT_FILE)) {
        shadowInputFile =
                FileUtils::ExtractExpandedPath(
                        cliArgs.at(PlainConfig::SampleShadow::CLI_SAMPLE_SHADOW_INPUT_FILE)).c_str();
    }
...
bool Config::ParseCliArgs(int argc, char **argv, CliArgs &cliArgs) {
    ...
    ArgumentDefinition argumentDefinitions[] = {
            {CLI_HELP,                                                                   false, true,  [](
                    const string &additionalArg) { PrintHelpMessage(); }},
            {CLI_VERSION,                                                                false, true,  [](
                    const string &additionalArg) { PrintVersion(); }},
            {CLI_EXPORT_DEFAULT_SETTINGS,
                                                                                         true,
                                                                                                true,
                                                                                                       [](const string &additionalArg) {
                                                                                                           ExportDefaultSetting(
                                                                                                                   additionalArg);
                                                                                                       }},
