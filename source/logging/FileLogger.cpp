...
void FileLogger::createLogDirectories() {
    #if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
        FileLogger::DEFAULT_LOG_DIR = "out\\logs\\";
        FileLogger::DEFAULT_LOG_FILE = "out\\logs\\dc-application.log";
    #endif
}
...
bool FileLogger::start() {
    ...
    if(stat(DEFAULT_LOG_DIR.c_str(), &info) != 0 ) {
        cout << LOGGER_TAG << ": Cannot access " << DEFAULT_LOG_DIR << "to write logs, attempting to create log directory" << endl;
...
void FileLogger::queueLog(LogLevel level, const char * tag, std::chrono::time_point<std::chrono::system_clock> t, string message) {
