...
constexpr int StdOutFileLogger::DEFAULT_WAIT_TIME_MILLISECONDS;
constexpr char StdOutFileLogger::DEFAULT_LOG_FILE[];

bool StdOutFileLogger::start(const PlainConfig &config)
{
    ...
    if (!outputStream->fail())
    {
        ...
        thread log_thread(&StdOutFileLogger::run, this);
        log_thread.detach();
        return true;
    }
...
void StdOutFileLogger::writeLogMessage(unique_ptr<LogMessage> message)
{
...
unique_ptr<LogQueue> StdOutFileLogger::takeLogQueue()
{
...
void StdOutFileLogger::setLogQueue(std::unique_ptr<LogQueue> incomingQueue)
{
    this->logQueue = std::move(incomingQueue);
}

void StdOutFileLogger::queueLog(
    LogLevel level,
    const char *tag,
...
void StdOutFileLogger::shutdown()
{
    needsShutdown = true;
    logQueue->shutdown();
